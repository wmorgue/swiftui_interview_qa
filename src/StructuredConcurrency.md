...
Система предполагает, что `executors` надеждны и никогда не откажутся от выполнения `job`.
...
Точная семантика обрабатываетки приоритета, зависит от конкретной платформы и конкретной реализации `executor`.
...
Любая задача переденная в UI thread будет выполняться с высоким приоритетом в течении всего времени выполнения.
Это помогает гарантировать, что UI thread будет доступен для выполнения более приоритетной `task`, если она будет представлена позжe.
...
- Если `task` создана во время обработки `task` и `task` с высоким проритетом ожидает завершения, то приоритет `task` будет повышен постоянно, чтобы соответствовать задаче с высоким приоритетом.
Не влияет на дочерние задачи.
