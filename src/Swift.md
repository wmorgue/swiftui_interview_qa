...
`Void` является typealias к `()`:

```swift
typealias Void = ()
```

`Void` является типом данных, а `()` одновременно является типом данных и значением:
...
print(type(of: Int.self))   // Int.Type
print(type(of: Void.self))  // ().Type
print(type(of: ().self))    // ()
```

## В чём разница между guard let и if let?

Оба `guard let` и `if let` разворачивают опциональные значения, если они имеются.
...
## Преимущества использовать guard let statement?

Два основных преимущества использовать `guard let`:
